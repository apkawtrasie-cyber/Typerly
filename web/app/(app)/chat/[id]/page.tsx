...
export default function ChatRoomPage() {
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-72px)]">
      {/* Header */}
      <div className="px-4 pt-5 pb-3 border-b border-white/[0.12] flex items-center gap-3">
        ...
        {messages.length === 0 ? (
          ...
          return (
            <div key={m.id} className={`flex flex-col ${mine ? "items-end" : "items-start"}`}>
              ...
              <div className={`px-4 py-2 rounded-2xl text-sm max-w-[80%] ${mine ? "bg-[#F5C400] text-black font-medium rounded-br-md" : "bg-[#1e1e1e] text-white rounded-bl-md"}`}>
                {m.content}
              </div>
            </div>
          );
      ...
      {/* Pole wpisywania */}
      <div className="px-4 py-3 border-t border-white/[0.12] flex gap-2"
        style={{ paddingBottom: "max(12px, env(safe-area-inset-bottom))" }}>
        <input value={text} onChange={e => setText(e.target.value)}
          onKeyDown={e => e.key === "Enter" && send()}
          placeholder={t("chat.placeholder")}
          className="flex-1 bg-[#1e1e1e] border border-white/[0.12] rounded-xl px-4 py-3 text-white placeholder-white/20 focus:outline-none focus:border-[#F5C400]/40" />
